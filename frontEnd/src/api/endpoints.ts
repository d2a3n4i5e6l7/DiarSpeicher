        ...
        permissions: [
            {
                ...
                description: "Dar de alta bibliotecas nuevas. Editarlas y borrarlas sigue siendo exclusivo del propietario del servidor.",
            },
...
export const usersApi = {
    ...
    changePassword: (id: number, newPassword: string) => gatewayHttp.put<{ updated: boolean }>(`/users/${id}`, { newPassword }),
...
export const rolesApi = {
    ...
    create: (payload: CreateRolePayload): Promise<{ id: number }> => Promise.resolve({ id: payload.name ? 99 : 0 }),
    update: (id: number, payload: UpdateRolePayload): Promise<{ updated: boolean }> => Promise.resolve({ updated: id > 0 && payload !== undefined }),
    delete: (id: number): Promise<void> => (id > 0 ? Promise.resolve() : Promise.reject(new Error("ID inválido"))),
};
...
export const filesystemApi = {
    deletionPreview: (path: string) => http.get<DeletionPreview>(`/api/v2/filesystem/deletion?path=${encodeURIComponent(path)}`),
    deleteEntry: (path: string) => http.delete<TrashEntry>(`/api/v2/filesystem/entry?path=${encodeURIComponent(path)}`),
    preview: (path: string, pattern: string) => http.get<ScanPreview>(`/api/v2/filesystem/preview?path=${encodeURIComponent(path)}&pattern=${encodeURIComponent(pattern)}`),
    disk: (path: string) => http.get<DiskUsage>(`/api/v2/filesystem/disk?path=${encodeURIComponent(path)}`),
    createFolder: (parent: string, name: string) => http.post<FolderEntry>("/api/v2/filesystem/folder", { parent, name }),
    roots: () => http.get<FolderRoot[]>("/api/v2/filesystem/roots"),
    browse: (path?: string) => http.get<FolderListing>(`/api/v2/filesystem/browse${path ? `?path=${encodeURIComponent(path)}` : ""}`),
...
export const librariesApi = {
    ...
    delete: (id: string, deleteFiles = false) => http.delete<void>(`/api/v2/libraries/${id}?deleteFiles=${String(deleteFiles)}`),
...
export const mediaApi = {
    ...
    delete: (id: string, deleteFile = false) => http.delete<void>(`/api/v2/media/${id}?deleteFile=${String(deleteFile)}`),
    updateProgress: (id: string, payload: ProgressPayload) => http.put<{ updated: boolean }>(`/api/v2/media/${id}/progress`, payload),
    ...
    latest: async (limit = 20): Promise<MediaItem[]> => {
        ...
        const page = await http.get<PageResponse<MediaItem>>(`/api/v2/media?${pageQuery(0, size)}&newestFirst=true`);
        return page.data;
    },
...
export const seriesApi = {
    ...
    delete: (id: string, deleteFiles = false) => http.delete<void>(`/api/v2/series/${id}?deleteFiles=${String(deleteFiles)}`),
    media: (id: string, page = 0, pageSize = 50) => http.get<PageResponse<MediaItem>>(`/api/v2/series/${id}/media?${pageQuery(page, pageSize)}`),
     ...
     */
    update: (id: string, payload: { name?: string; description?: string }) => http.put<SeriesItem>(`/api/v2/series/${id}`, payload),
     ...
     */
    thumbnailUrl: (id: string, token?: string | null) => `${API_BASE}/api/v2/series/${id}/thumbnail${token ? `?v=${encodeURIComponent(token)}` : ""}`,

    /** Adopta como portada la miniatura de un tomo ya indexado. */
    setCoverFromMedia: (seriesId: string, mediaId: string) => http.put<{ updated: boolean }>(`/api/v2/series/${seriesId}/thumbnail`, { mediaId }),
...
export const metadataApi = {
     ...
     */
    coverUrl: (externalUrl: string) => `${API_BASE}/api/v2/metadata/cover?url=${encodeURIComponent(externalUrl)}`,
};
