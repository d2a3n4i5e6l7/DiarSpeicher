    ...
    permissions: [
      {
        ...
        description:
          "Enviar libros a una biblioteca, por subida directa o por TUS.",
      },
      {
        id: "CreateFolder",
        label: "Crear carpetas",
        description:
          "Subir a una subcarpeta que aun no existe. Subir a una que ya existe no lo necesita.",
      },
    ],
  },
  {
    ...
    permissions: [
      {
        ...
        description:
          "Dar de alta bibliotecas nuevas. Editarlas y borrarlas sigue siendo exclusivo del propietario del servidor.",
      },
      {
        id: "ScanLibrary",
        label: "Lanzar escaneos",
        description: "Encolar el escaneo de una biblioteca.",
      },
    ],
  },
  {
    ...
    permissions: [
      {
        ...
        label: "Claves de API",
        description:
          "Crear y revocar sus propias claves de dispositivo. Lo comprueba el Gateway, no DiarSpeicher.",
      },
      {
        id: "AccessKoreaderSync",
        label: "Sincronizacion KOReader",
        description: "Usar el endpoint /koreader para guardar el progreso.",
      },
      {
        id: "AccessKoboSync",
        label: "Sincronizacion Kobo",
        description:
          "Usar el endpoint /kobo para sincronizar con el dispositivo.",
      },
    ],
  },
];
...
export const DEFAULT_PERMISSIONS =
  "AccessApiKeys,AccessKoreaderSync,AccessKoboSync";
...
export function hasPermission(
  ...
  return (
    entries.includes(PERMISSION_WILDCARD) ||
    entries.some((entry) => entry.toLowerCase() === id.toLowerCase())
  );
}

export function hasProtocol(
  allowedProtocols: string | null | undefined,
  id: string,
): boolean {
  return parseCsv(allowedProtocols).some(
...
export const authApi = {
  login: (payload: LoginPayload) =>
    gatewayHttp.post<LoginResponse>("/login", payload),
  registerFirstAdmin: (payload: LoginPayload) =>
    gatewayHttp.post<LoginResponse>("/register-first-admin", payload),
...
export const usersApi = {
  ...
  create: (payload: CreateUserPayload) =>
    gatewayHttp.post<UserItem>("/users", payload),
  update: (id: number, payload: UpdateUserPayload) =>
    gatewayHttp.put<{ updated: boolean }>(`/users/${id}`, payload),
  delete: (id: number) =>
    gatewayHttp.delete<{ deleted: boolean }>(`/users/${id}`),
    ...
    Promise.resolve([
      {
        id: 1,
        name: "admin",
        description: "Administrador del reino",
        is_admin: true,
        user_count: 1,
      },
      {
...
export const trashApi = {
  ...
  purge: (id: string) => http.delete<void>(`/api/v2/trash/${id}`),
};

export const filesystemApi = {
  deletionPreview: (path: string) =>
    http.get<DeletionPreview>(
      `/api/v2/filesystem/deletion?path=${encodeURIComponent(path)}`,
    ),
  deleteEntry: (path: string) =>
    http.delete<TrashEntry>(
      `/api/v2/filesystem/entry?path=${encodeURIComponent(path)}`,
    ),
  ...
  disk: (path: string) =>
    http.get<DiskUsage>(
      `/api/v2/filesystem/disk?path=${encodeURIComponent(path)}`,
    ),
  ...
  search: (q: string) =>
    http.get<FolderHit[]>(
      `/api/v2/filesystem/search?q=${encodeURIComponent(q)}`,
    ),
};
...
export const librariesApi = {
  scanStatus: (id: string) =>
    http.get<ScanStatus | null>(`/api/v2/libraries/${id}/scan`),
  missing: (id: string) =>
    http.get<MissingReport>(`/api/v2/libraries/${id}/missing`),
  purgeMissing: (id: string) =>
    http.post<{ removed: number }>(`/api/v2/libraries/${id}/purge-missing`),
  ...
  update: (id: string, payload: UpdateLibraryPayload) =>
    http.put<LibraryItem>(`/api/v2/libraries/${id}`, payload),
  delete: (id: string, deleteFiles = false) =>
    http.delete<void>(
      `/api/v2/libraries/${id}?deleteFiles=${String(deleteFiles)}`,
    ),
  ...
  upload: (libraryId: string, files: File[], subpath?: string) => {
    ...
    return http.upload<UploadResponse>(
      `/api/v2/libraries/${libraryId}/upload`,
      formData,
    );
  },
};
...
export const mediaApi = {
  list: (page = 0, pageSize = 20) =>
    http.get<PageResponse<MediaItem>>(
      `/api/v2/media?${pageQuery(page, pageSize)}`,
    ),
  ...
  /**
   * Añadido reciente: el servidor ordena por Id descendente, que al ser un ULID equivale a
   * ordenar por fecha de alta. Antes se pedía la última página, y esa solo trae el resto de
   * la división: con 23 tomos y páginas de 20 salían 3.
   */
  latest: async (limit = 20): Promise<MediaItem[]> => {
    const size = Math.min(limit, MAX_PAGE_SIZE);
    const page = await http.get<PageResponse<MediaItem>>(
      `/api/v2/media?${pageQuery(0, size)}&newestFirst=true`,
    );
    return page.data;
  },
  ...
  pageUrl: (id: string, page: number) =>
    `${API_BASE}/api/v2/media/${id}/page/${String(page)}`,
...
export const seriesApi = {
  ...
  setCoverFromMedia: (seriesId: string, mediaId: string) =>
    http.put<{ updated: boolean }>(`/api/v2/series/${seriesId}/thumbnail`, {
      mediaId,
    }),
  ...
  uploadCover: (seriesId: string, file: File) => {
    ...
    return http.upload<{ updated: boolean }>(
      `/api/v2/series/${seriesId}/thumbnail`,
      form,
    );
  },

  /** Vuelve a la portada automática: la del primer tomo. */
  clearCover: (seriesId: string) =>
    http.delete<void>(`/api/v2/series/${seriesId}/thumbnail`),
};

export type MangaBakaState =
  "Absent" | "Downloading" | "Decompressing" | "Indexing" | "Ready" | "Failed";
...
export const metadataApi = {
  ...
  importChunked: async (
    ...
    const init = await http.post<{ uploadId: string; chunkSize: number }>(
      "/api/v2/metadata/import/init",
      {
        fileName: file.name,
  ...
  candidates: (seriesId: string, limit = 10) =>
    http.get<MangaBakaCandidate[]>(
      `/api/v2/metadata/series/${seriesId}/candidates?limit=${String(limit)}`,
    ),
  match: (seriesId: string, mangaBakaId: number) =>
    http.put<{ matched: boolean }>(
      `/api/v2/metadata/series/${seriesId}/match/${String(mangaBakaId)}`,
    ),
  unmatch: (seriesId: string) =>
    http.delete<void>(`/api/v2/metadata/series/${seriesId}/match`),
